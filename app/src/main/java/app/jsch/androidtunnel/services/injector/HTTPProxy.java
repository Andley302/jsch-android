...
public class HTTPProxy
{
	...
	public Socket socket() {
        ...
        try {
			...
			int port = AppPrefs.getProxyPort();
			String payload = "CONNECT [host_port] [protocol][crlf][crlf]";
            socket = new Socket(servidor, port);
			payload = AppPrefs.getPayloadKey();
            AppLogManager.addToLog("<b><font color=#49C53C>Enviando HTTP....</font></b>");
            ...
            if (replace.contains("[split]")) {
                ...
                for (i2 = 0; i2 < split3.length; i2++) {
                   //	AppLogManager.addToLog(split3[i2].replace("\r", "\\r").replace("\n", "\\n"));
                ...
                AppLogManager.addToLog(replace.replace("\r", "\\r").replace("\n", "\\n"));
                outputStream.write(replace.getBytes());
                outputStream.flush();
            }
           AppLogManager.addToLog("HTTP: Sending payload");
        } catch (Exception e2) {
            AppLogManager.addToLog("HTTP: Fail to remote proxy");
        }
        countRotate++;
        countRotate2++;
        countRotate3++;
    ...
    private String auth() {
        ...
        try {
           ...
           AppLogManager.addToLog(e.getMessage());
        }
        return str;
    }
        ...
        try {
            ...
            while (true) {
				...
				if (stringBuilder.toString().equals("")) {
					AppLogManager.addToLog("Get request data failed, empty requestline");
					return null;
				}
						String c = c(stringBuilder.toString());
				if (c != null) {
					AppLogManager.addToLog("Try connect...");
					//h.a("Socket Server", "Connecting to " + str);
					Socket socket = new Socket();
                    String ip = AppPrefs.getProxyIP();
                    int port = AppPrefs.getProxyPort();
					socket.connect(new InetSocketAddress(ip, Integer.valueOf(port).intValue()));
					a(c, socket);
					return socket;
				}
				return null;
        ...
        if (str != null) {
            try {
                if (!str.equals("")) {
                    ...
                    f = split2[0];
                    i = split2[1];
                    str2 = d(AppPrefs.getPayloadKey()).replace("[real_raw]", str).replace("[raw]", charSequence).replace("[method]", split[0]).replace("[host_port]", split[1]).replace("[host]", f).replace("[port]", i).replace("[protocol]", split[2])/*.replace("[auth]", this.j)*/.replace("[ua]", ua()).replace("[cr]", "\r").replace("[lf]", "\n").replace("[crlf]", "\r\n").replace("[lfcr]", "\n\r").replace("\\r", "\r").replace("\\n", "\n");
                    return str2;
                }
            } catch (Exception e) {
	...
	public Socket socket2() {
        ...
        //ipv6 fix
        if (!ResolveAddrs.checkIsIPV6(AppPrefs.getProxyIP())){
            String remote = AppPrefs.getProxyIP() + ":" + AppPrefs.getProxyPort();
            split = remote.trim().split(":");
        }
        int i2 = AppPrefs.getProxyPort();;
        trim = AppPrefs.getProxyIP();
        ...
        try {
			...
			//AppLogManager.addToLog("There was an error connecting.");
			//AppLogManager.addToLog("HTTP Proxy Error: " + e22);
            AppLogManager.addToLog("<strong><font color=#FF0000>VPN: </strong></font>" + e22.toString());

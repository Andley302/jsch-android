...
public class SSLProxy {
  ...
  public Socket inject() {
    try {
      ...
      BufferedReader bufferedReader = new BufferedReader(
        new InputStreamReader(this.incoming.getInputStream())
      );
  ...
  /*private SSLSocket doSSLHandshake(String host, String sni, int port) throws IOException {
        try {
            ...
            try{
                ...
                AppLogManager.addToLog("Error: " + e);
                e.printStackTrace();
            }
            ...
            AppLogManager.addToLog("Starting SSL Handshake...");
  ...
  private SSLSocket doSSLHandshake(String host, String sni, int port)
    throws IOException {
    ...
    try {
      ...
      socket.addHandshakeCompletedListener(
        new SSLProxy.mHandshakeCompletedListener(host, port, socket)
      );
      ...
      IOException iOException = new IOException(
        new StringBuffer()
          .append("Could not complete SSL handshake: ")
          .append(e)
          .toString()
      );
      throw iOException;
      ...
      if (matcher.find()) {
        ...
        str =
          str8.replace(
            new StringBuffer()
              .append(str2)
              .append(String.valueOf(intValue))
              .append("]")
              .toString(),
            str7
          );
        ...
        if (!str.equals("")) {
          ...
          str2 =
            d(
              AppPrefs
                .getPayloadKey()
                .replace("[real_raw]", str)
                .replace("[raw]", charSequence)
                .replace("[method]", split[0])
                .replace("[host_port]", split[1])
                .replace("[host]", f)
                .replace("[port]", i)
                .replace("[protocol]", split[2])
                /*.replace("[auth]", auth())*/.replace("[ua]", ua())
                .replace("[cr]", "\r")
                .replace("[lf]", "\n")
                .replace("[crlf]", "\r\n")
                .replace("[lfcr]", "\n\r")
  ...
  private void InjectPayload(String str, Socket socket) throws Exception {
    AppLogManager.addToLog("<b><font color=#49C53C>Sending HTTP...</font></b>");
    int i = 0;
    String[] split;

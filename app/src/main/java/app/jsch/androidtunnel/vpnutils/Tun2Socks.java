...
public class Tun2Socks extends Thread implements StreamGobbler.OnLineListener {
	...
	public void run() {
		...
		try {
			...
			if (mVpnInterfaceFileDescriptor != null){
				...
				if (mUdpgwServerAddress != null) {
					if (mUdpgwTransparentDNS) {
						...
						AppLogManager.addToLog("udpgw transparent dns");
					}else{
						AppLogManager.addToLog("udpgw addr: " + mUdpgwServerAddress);
					}
			...
			////ServiceManagerModule.logException("Tun2Socks Error", e);
		} catch (Exception e) {
			////ServiceManagerModule.logDebug("Tun2Socks Error: " + e.getMessage());
		}
	...
	public void onLine(String log){
		////ServiceManagerModule.logDebug("Tun2Socks: " + log);
	}
	...
	private boolean sendFd(ParcelFileDescriptor fileDescriptor, File toFile) throws InterruptedException {
		...
		////ServiceManagerModule.logDebug("Enviando Fd para sock");
